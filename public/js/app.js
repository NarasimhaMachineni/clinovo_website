      ...
      app.showPharmaLoader('Submitting Feasibility Questionnaire & Calculating SQLite 3 Scores...', async () => {
        try {
          ...
          if (data.success) {
            ...
            showToast(`Site saved to SQLite 3 DB! Overall Score: ${data.overallScore}/100`);
            app.navigateTo('dashboard');
          } else {
            showToast('Submission error: ' + data.message);
          }
        } catch (err) {
  ...
  const dashApp = {
    ...
    async fetchSites() {
      try {
        ...
        console.error('Error fetching sites from SQLite 3:', err);
      }
    ...
    async saveModal() {
      ...
      try {
        ...
        console.error('Error saving site to SQLite 3:', err);
      }

      this.closeModal();
      this.renderAll();
      showToast('Site saved to SQLite 3 DB');
    },

    async deleteSite() {
      if (!this.editingId) return;
      if (!confirm('Are you sure you want to delete this site from SQLite 3?')) return;

      try {
        const res = await fetch(`/api/sites/${this.editingId}`, { method: 'DELETE' });
        const data = await res.json();
        if (data.success) state.sites = data.sites;
      } catch (err) {
        console.error('Error deleting site from SQLite 3:', err);
      }

      this.radarSelected.delete(this.editingId);
      this.closeModal();
      this.renderAll();
      showToast('Site deleted from SQLite DB');
    },
